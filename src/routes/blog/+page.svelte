...
<main class="bg-primary min-h-screen pt-20">
	...
	<section class="bg-primary pt-32 pb-10">
		<div class="container mx-auto px-6 text-center">
			<h1 class="text-card text-4xl font-bold md:text-5xl lg:text-6xl">Blog</h1>
		</div>
	</section>
			...
			{#if data.posts.length > 0}
				<div class="space-y-6">
					{#each data.posts as post}
						<article class="bg-card-dark rounded-lg p-4 transition-all hover:scale-[1.02] hover:shadow-lg">
							<a href="/blog/{post.id}" class="block">
								<div class="flex gap-4">
									...
									<div class="flex-1 overflow-hidden">
										...
										<h2 class="text-card text-lg font-bold leading-tight md:text-xl">
											{post.title}
										</h2>
									</div>
								</div>
